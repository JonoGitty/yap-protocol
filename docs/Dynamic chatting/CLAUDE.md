...
### The Branch (Thread)
A conversation thread between agents. Lifecycle:
`INITIATED → NEGOTIATING ⇄ (loops) → PROPOSED → CONFIRMED → EXECUTING → COMPLETED`
...
A popup/message to the user asking permission to share specific context.
Should be: one-tap, contextualised (show WHY it's being asked), batched.

### Dynamic Schema Negotiation
When the base freeform context isn't structured enough for a complex task, agents can negotiate a custom schema on the fly. One agent proposes a set of typed fields, the other reviews/modifies/accepts, and then both sides fill in the schema — yapping back and forth until every field is filled, every conflict is resolved, and zero context is missing. Schemas can include service integrations (e.g. "should we create a Spotify collaborative playlist for this road trip?"). Proven schemas get cached in flock memory and reused. See `docs/DYNAMIC_SCHEMAS.md` for the full spec.
...
2. `docs/DYNAMIC_SCHEMAS.md` — How agents negotiate custom schemas and service integrations.
3. `docs/VOCABULARY.md` — The naming conventions.
4. `docs/ARCHITECTURE.md` — Why the architecture is the way it is.
5. This file — For quick reference while building.
